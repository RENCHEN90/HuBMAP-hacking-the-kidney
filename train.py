...
def parnet_train(epoch, model, optimizer, train_loader, val_loader=None, device='cuda'):
...
def train(epoch,model,optimizer,train_loader,val_loader=None,device='cuda'):
    ...
    for i, pack in tqdm(enumerate(train_loader),total = len(train_loader),ncols=100):
        for rate in size_rates:
            ...
            #  loss function
            loss = opt.criterion(out_flat,gts_flat)    # TODO: try different weights for loss
            epoch_loss += loss.item()
